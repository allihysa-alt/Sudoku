...
// G-03 — `showCalculatedCandidates` was the pre-collapse name for the
// "fill every empty cell with its full legal candidate set" tool. That
// tool is now `toggleShowCandidates`, and the candidates land in the
// transient `autoOuter` layer instead of the manual `innerPencils` /
// `currentSnapshot`. The helper below mirrors the original snapshot
// format so the assertions below keep their original shape.
function autoCandsSnapshot(grid) {
    return grid.get('cells').filter(c => c.get('autoOuter').size > 0)
        .map(c => {
            const cs = c.get('autoOuter').toArray().sort().join('');
            return `${c.get('row')}${c.get('col')}N${cs}`;
        })
        .toArray()
        .join(',');
}
...
test('check calculateCellCandidates() no SN', () => {
    ...
    const grid2 = modelHelpers.toggleShowCandidates(grid);
    expect(autoCandsSnapshot(grid2)).toBe(
