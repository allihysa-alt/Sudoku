...
// C:\\Users\\ismoo\\Desktop\\sudoku-web-app-master\\src\\lib\\sudoku-explainer.js.
// Imported by sudoku-model.ts; out-of-UI-scope for v1 per proposal §18,
// but kept for engine coherence so tests resolve cleanly.
...
class SudokuExplainer {
    ...
    inflate(sparseSteps) {
        ...
        const steps = sparseSteps.map((s) => {
            const step = {
                ...
                title: s.sd,
                html: s.ht,
                // Batch-5 — two-stage hint disclosure. May be undefined
                // for legacy analysis payloads that pre-date the nudge
                // fields; the modal falls back to "Hint" / the full
                // title in that case.
                nudgeTitle: s.nt,
                nudge: s.nb,
            };
    ...
    findNextStep(currDigits, currCandidates) {
        ...
        // P12a-fix: previously this threw "Failed to find hint step",
        // which crashed React to a white screen when the player
        // applied the final hint via Apply & Next (the puzzle was
        // already solved, so no step matched). Returning null lets
        // showHintModal fall through to its existing noHint branch.
        return null;
    }
