...
// src/components/sudoku-grid/sudoku-cell-region-outline.js.
// className passed in by the caller (e.g. selectionOutline from the
// SudokuGrid module) so this stays purely structural — no styling
// decisions live here.
...
export default function SudokuCellRegionOutline({ className, cellSet, dim }) {
  ...
  const pathData = calculateOutlinePath(cellSet)
    .map((step) => {
      const [i, p, op] = step;
    ...
    .join(' ');
  return (
    <path className={className} d={pathData} strokeLinecap="round" strokeLinejoin="round" />
  );
}
